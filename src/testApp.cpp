...
void testApp::setup(){
	...
	markerSize = 0.15;
	//__string boardName = "boardLogo.yml";
	aruco.setup("intrinsics.int", grabber.getWidth(), grabber.getHeight(), "", markerSize);
	...
	terreno.set (terrainBG.width*2, terrainBG.height*2, 150, 100, OF_PRIMITIVE_TRIANGLES);
	vector<ofVec3f> & terrenoVertices = terreno.getMesh().getVertices();
	for (int i=0, y=0; y< 100; y++){
		for(int x=0; x<150; x++,i++){
			float brx = x/150.* terrainBG.width;
			float bry = y/100.* terrainBG.height;
			float br = terrainBG.getColor(brx,bry).getBrightness();
			terrenoVertices[i].z += terrenoVertices[i].z + br * 2.5;
	...
	LabelNetescopio::libRocket->loadFont("FreeSansOblique.ttf");
...
void testApp::update(){
	...
	if(state==AR){
		...
		if (grabber.isFrameNew()){
			...
			//__aruco.detectBoard(grabber.getPixelsRef());
			...
			aruco.detectMarkers(grabber.getPixelsRef());
		}

		/*__if(aruco.getBoardProbability()>.03){
			logo.update();
		}*/
		...
		for(int i=0;i<aruco.getNumMarkers();i++){
			...
			if(particles.find(currentId)!=particles.end()){
				...
				if(lastSeenMarker.find(aruco.getMarkers()[i].idMarker)==lastSeenMarker.end()){
					//cout << "found " << aruco.getMarkers()[i].idMarker << endl;
					ofLogNotice() << "foundID: " << currentId << endl;
...
void testApp::draw(){
	...
	if(state==AR){
		...
		for(int i=0;i<aruco.getNumMarkers();i++){
			currentId = aruco.getMarkers()[i].idMarker;
			//cout << "---" << endl;
			aruco.begin(i);
			...
			if(particles.find(currentId)!=particles.end()){
				...
				float sx = 1/float(logo.width)*markerSize;
				ofScale(sx,sx,sx);
				ofRotateX(-90);
				ofRotateY(180);
				ofTranslate(-(logo.width+100)*.5,-(logo.height+100)*.5,0);
				ofSetColor(255);

				logo.draw(0,0,logo.width+100,logo.height+100);
				ofPopMatrix();

				ofPushMatrix();
				ofScale(0.00025,0.000375,0.000375);
				ofRotateX(-30);
				ofTranslate(0,3000,500);
				...
				for (int i=0; i<35; i++){
					...
					ofLine(ofVec3f(
								ofRandom(-markerSize*.5),ofRandom(-markerSize*.5),ofRandom(-markerSize*.5)
						   ),
						   ofVec3f(
								ofRandom(markerSize*-250),ofRandom(markerSize*-250),ofRandom(markerSize*-250)
						   )
					);
	...
	//ofDrawBitmapString(ofToString(currentId),10,30);
