...
string letra[] = {"<avatar>","<cache>","<camuflaje>", "<copyright>",
...
void Emitter::setup(ofVec3f location){
	...
	font.loadFont("Akkurat-Mono.otf",25);
	...
	for(int i=0;i<24;i++){
		words.push_back(font.getStringMesh(letra[i],0,0));
	}
...
void Emitter::addLetters(){
	...
	if(prob<.7){
		letter.setup(ofRandom(0.5,2), origin, words[(int)ofRandom(2)],font.getFontTexture());
	}else{
		letter.setup(ofRandom(0.5,2), origin, words[(int)ofRandom(3,24)],font.getFontTexture());
	}
...
void Emitter::addImages(){
	...
	//int m_ = m;
	//if (m_==0){
	image.setup(ofRandom(0.5,2), origin, img[(int)ofRandom(20)]);
